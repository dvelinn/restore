...
# A little configuration for greetd
info "Configuring tuigreet session manager"

# Make sure tuigreet is actually installed
sudo pacman --noconfirm -S greetd-tuigreet

# Copy config files
sudo rm /etc/greetd/config.toml
sudo cp $HOME/Documents/Scripts/greetd/config.toml /etc/greetd/config.toml
